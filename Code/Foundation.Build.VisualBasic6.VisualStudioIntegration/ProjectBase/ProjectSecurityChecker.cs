...
{
    using Url = Shell11.Microsoft.VisualStudio.Shell.Url;
		...
		protected virtual bool IsProjectSafeWithImports(out string securityErrorMessage)
		{
			...
			if(directImports != null && directImports.Length > 0)
			{
				...
				for(int i = 0; i < directImports.Length; i++)
				{
					...
					{
						using(RegistryKey root = Shell11.Microsoft.VisualStudio.Shell.VSRegistry.RegistryRoot(__VsLocalRegistryType.RegType_Configuration))
						{
